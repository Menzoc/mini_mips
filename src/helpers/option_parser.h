/*
This file is based from the project under MIT-LICENSE ezOptionParser (Copyright (C) 2011,2012,2014 Remik Ziemlinski <first d0t surname att gmail>).
*/
...
namespace optpars {
    ...
    /*
     * Makes a vector of strings from one string, splitting at (and excluding) delimiter "token".
     */
     ...
     * - or see if within set of given list of values.s
    ...
    public:
        inline OptionValidator(const char *_type, const char *_op = 0, const char *list = 0, bool _insensitive = false);

        inline OptionValidator(char _type);

        inline OptionValidator(char _type, char _op, const char *list, int _size);

        inline OptionValidator(char _type, char _op, const unsigned char *list, int _size);

        inline OptionValidator(char _type, char _op, const short *list, int _size);

        inline OptionValidator(char _type, char _op, const unsigned short *list, int _size);

        inline OptionValidator(char _type, char _op, const int *list, int _size);

        inline OptionValidator(char _type, char _op, const unsigned int *list, int _size);

        inline OptionValidator(char _type, char _op, const long long *list, int _size);

        inline OptionValidator(char _type, char _op, const unsigned long long *list, int _size = 0);

        inline OptionValidator(char _type, char _op, const float *list, int _size);

        inline OptionValidator(char _type, char _op, const double *list, int _size);

        inline OptionValidator(char _type, char _op, const char **list, int _size, bool _insensitive);

        inline ~OptionValidator();
...
#define CLEAR(TYPE, P) case TYPE: if (P) delete [] P; P = 0; break;
    ...
    OptionValidator::OptionValidator(char _type, char _op, const char *list, int _size) : s1(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = OptionParserIDGenerator::instance().next();
        s1 = new char[size];
        memcpy(s1, list, size);
    };

    OptionValidator::OptionValidator(char _type, char _op, const unsigned char *list, int _size) : u1(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = OptionParserIDGenerator::instance().next();
        u1 = new unsigned char[size];
        memcpy(u1, list, size);
    };

    OptionValidator::OptionValidator(char _type, char _op, const short *list, int _size) : s2(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = OptionParserIDGenerator::instance().next();
        s2 = new short[size];
        memcpy(s2, list, size * sizeof(short));
    };

    OptionValidator::OptionValidator(char _type, char _op, const unsigned short *list, int _size) : u2(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = OptionParserIDGenerator::instance().next();
        u2 = new unsigned short[size];
        memcpy(u2, list, size * sizeof(unsigned short));
    };

    OptionValidator::OptionValidator(char _type, char _op, const int *list, int _size) : s4(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = OptionParserIDGenerator::instance().next();
        s4 = new int[size];
        memcpy(s4, list, size * sizeof(int));
    };

    OptionValidator::OptionValidator(char _type, char _op, const unsigned int *list, int _size) : u4(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = OptionParserIDGenerator::instance().next();
        u4 = new unsigned int[size];
        memcpy(u4, list, size * sizeof(unsigned int));
    };

    OptionValidator::OptionValidator(char _type, char _op, const long long *list, int _size) : s8(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = OptionParserIDGenerator::instance().next();
        s8 = new long long[size];
        memcpy(s8, list, size * sizeof(long long));
    };

    OptionValidator::OptionValidator(char _type, char _op, const unsigned long long *list, int _size) : u8(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = OptionParserIDGenerator::instance().next();
...
#define ToArray(T, P, Y) case T: P = new Y[size]; To##T(strings, P, size); FreeStrings(); break;
    ...
    bool OptionValidator::isValid(const std::string *valueAsString) {
...
#define CHECKRANGE(E, T) {\
  ...
  if (E##value > E##max) { \
    ...
    return false; \
        ...
        if (type != T) {
            switch (type) {
                case S1: CHECKRANGE(S1, char);
                    break;
                case U1: CHECKRANGE(U1, unsigned char);
                    break;
                case S2: CHECKRANGE(S2, short);
                    break;
                case U2: CHECKRANGE(U2, unsigned short);
                    break;
                case S4: CHECKRANGE(S4, int);
                    break;
                case U4: CHECKRANGE(U4, unsigned int);
                    break;
                case S8: {
  ...
  switch (op) {\
    ...
    case GTLE:\
      ...
      case NOOP: case IN: default: break;\
        ...
        switch (type) {
            case U1: VALIDATE(unsigned char, int, u1);
                break;
            case S1: VALIDATE(char, int, s1);
                break;
            case U2: VALIDATE(unsigned short, int, u2);
                break;
            case S2: VALIDATE(short, int, s2);
                break;
            case U4: VALIDATE(unsigned int, unsigned int, u4);
                break;
            case S4: VALIDATE(int, int, s4);
                break;
            case U8: VALIDATE(unsigned long long, unsigned long long, u8);
                break;
            case S8: VALIDATE(long long, long long, s8);
                break;
            case F: VALIDATE(float, float, f);
                break;
            case D: VALIDATE(double, double, d);
                break;
            default:
                break;
        }
    ...
    typedef std::map<int, OptionValidator *> ValidatorMap;
    ...
    public:
        ...
        inline void add(const char *defaults, bool required, int expectArgs, char delim, const char *help, const char *flag1, OptionValidator *validator = 0);

        inline void add(const char *defaults, bool required, int expectArgs, char delim, const char *help, const char *flag1, const char *flag2, OptionValidator *validator = 0);

        inline void add(const char *defaults, bool required, int expectArgs, char delim, const char *help, const char *flag1, const char *flag2, const char *flag3, OptionValidator *validator = 0);

        inline void add(const char *defaults, bool required, int expectArgs, char delim, const char *help, const char *flag1, const char *flag2, const char *flag3, const char *flag4, OptionValidator *validator = 0);
    ...
    bool OptionParser::exportFile(const char *filename, bool all) {
        int i;
        std::string out;
        bool quote;
     ...
     * So if this is used before parsing CLI, then option values will reflect this file, but if used after parsing CLI, then values will contain both CLI values and file's values.
     * Comment lines are allowed if prefixed with #.
     * Strings should be quoted as usual.
     */
    bool OptionParser::importFile(const char *filename, char comment) {
    ...
    int OptionParser::isSet(std::string &name) {
    ...
    void OptionParser::getUsage(std::string &usage, int width, Layout layout) {
     ...
     */
    void OptionParser::getUsageDescriptions(std::string &usage, int width, Layout layout) {
    ...
    bool OptionParser::gotExpected(std::vector<std::string> &badOptions) {
        int i, j;
    ...
    bool OptionParser::gotRequired(std::vector<std::string> &badOptions) {
        int i;
    ...
    bool OptionParser::gotValid(std::vector<std::string> &badOptions, std::vector<std::string> &badArgs) {
    ...
    void OptionParser::prettyPrint(std::string &out) {
        char tmp[256];
        int i, j, k;
